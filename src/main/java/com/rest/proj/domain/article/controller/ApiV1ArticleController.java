...
public class ApiV1ArticleController {
    ...
    public RsData<ArticlesResponse> getArticles() {
        List<ArticleDto> articles = articleService.getList().stream().map(article -> new ArticleDto(article)).toList();
    ...
    public RsData<ArticleResponse> getArticle(@PathVariable("id") Long id) {
        return articleService.getArticle(id).map(article -> RsData.of("S-1", "성공", new ArticleResponse(new ArticleDto(article)))).orElseGet(() -> RsData.of("F-1", "%d번 게시물은 존재하지 않습니다.".formatted(id), null));
    }
    ...
    public RsData modify(@Valid @RequestBody ModifyRequest modifyRequest, @PathVariable("id") Long id) {
        ...
        return RsData.of(modifyRs.getResultCode(), modifyRs.getMsg(), new ModifyResponse(modifyRs.getData()));
    }
    ...
    public RsData<RemoveResponse> remove(@PathVariable("id") Long id) {
        ...
        return RsData.of(deleteRs.getResultCode(), deleteRs.getMsg(), new RemoveResponse(deleteRs.getData()));
    }
